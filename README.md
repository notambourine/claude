...
| `nt-pm` | `/nt-pm:shipped` | Writes a plain-English "Deploy Updates" summary for Slack — what's about to ship (promotion or current branch vs the default branch) or what just shipped (the last push to the default branch, from the reflog), grouped by category for a non-technical audience. Never posts, never deploys. |
