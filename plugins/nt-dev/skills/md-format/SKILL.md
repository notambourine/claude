...
argument-hint: "[all | <file.md> | <dir>] [--width 80|100|200|never|keep] [--check]"
---
...
| "don't wrap", "one line per paragraph"        | `--width never`       |
...
- **`never`** for prose an agent will mirror, where one paragraph must be one physical
  line. `plugins/nt-dev/skills/pr/SKILL.md` is written that way on purpose.
